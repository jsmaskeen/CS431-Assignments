...
## Writeup: [`main.pdf`](./writeup/main.pdf)

For the fourth assignment, we will continue to solve CTF problems on 10.0.118.48:8000.

Create an account on the server and solve the four basic crypto-related problems. If you are not able to access the server, let us know. Please ensure that you are on IITGN-SSO when trying to access this. The assignments are done individually so you need to submit your own writeup for the assignment. 

Submit a one-page PDF writeup (except source code) with the following details:

1. For CTF problems, you must use the following format in your explanation:
    - CTF Username
    - Flag
...
4. Post any clarifications or questions regarding this in Canvas Discussions
...
6. Lastly, **you should not share flags or solutions**; we are logging your activity on the servers --- we will award 0 if we detect so.